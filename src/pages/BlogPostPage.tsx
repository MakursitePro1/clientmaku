...
const BlogPostPage = () => {
  ...
  if (relatedPosts.length < 2) {
    const more = blogPosts.filter((p) => p.id !== post.id && !relatedPosts.includes(p)).slice(0, 2 - relatedPosts.length);
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead title={post.title} description={post.excerpt} path={`/blog/${post.id}`} type="article" />
      <Navbar />
        ...
        <section className="px-4 pb-20">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {relatedPosts.map((rp) => (
                <Link key={rp.id} to={`/blog/${rp.id}`} className="group block rounded-2xl border border-border/30 bg-card/60 overflow-hidden hover:border-primary/30 transition-all">
                  <img src={rp.image} alt={rp.title} className="w-full h-36 object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                  <div className="p-4">
